...
1. Build the firmware:
   ...
   ```
2. Launch it on the device (it switches USB to dual-CDC and re-enumerates once —
   that brief port drop is expected):
   ```sh
   run-fap firmware/target/thumbv7em-none-eabihf/release/flip_link.fap
   ```
   (`run-fap` comes from `flipperzero-tools`: `cargo install --locked flipperzero-tools`.)
3. From the repo root, exercise the full CLI → daemon → device path:
   ```sh
   cargo build && ./target/debug/flip status
   ```
   Expected:
   ```
   daemon: up
